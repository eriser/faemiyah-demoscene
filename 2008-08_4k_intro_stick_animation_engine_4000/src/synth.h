...
const int16_t instruments[(VOICES_NORMAL + 1) * INSTR_LEN] = {
    // bd
...
void writeaudio(sample_t *out) {
	...
	for (i = SONGSTART; i < SONGLEN; i++) {
	    ...
	    // set pattern pointer
	    if (cflag) {
		instr = (int16_t*)(instruments + VOICES_NORMAL * INSTR_LEN);
		pattern = (uint8_t*)(patterns + PATTLEN * song[i + SONGLEN * VOICES_NORMAL]);
	    } else {
		instr = (int16_t*)(instruments + (v * INSTR_LEN));
		pattern = (uint8_t*)(patterns + PATTLEN * song[i + SONGLEN * v]);
	    }
