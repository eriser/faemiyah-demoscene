...
static void videocore_create_native_window(int screen_width, int screen_height,
    enum WindowMode mode, EGL_DISPMANX_WINDOW_T *out_native_window)
{
  ...
  switch(mode)
  {
    ...
    case MODE_KEEP_ASPECT:
      {
        const float rx = (float)native_width/screen_width;
        const float ry = (float)native_height/screen_height;
        const float r = (rx>ry) ? ry : rx;
        dst_rect.width = screen_width * r;
        dst_rect.height = screen_height * r;
